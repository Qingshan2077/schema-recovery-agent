...
def schema_recovery_v2() -> WorkflowDefinition:
    ...
    return WorkflowDefinition(
        ...
        nodes=[
            ...
            WorkflowNode(node_id="memory_retrieve", stage_id="memory.retrieve", depends_on=["survey"], required=False, writes=["output_refs", "errors"]),
            WorkflowNode(node_id="plan_work", depends_on=["memory_retrieve"], writes=["pending_work_units"]),
            ...
            WorkflowNode(node_id="memory_verify", stage_id="memory.verify", depends_on=["validate_join"], required=False, writes=["evidence_ids", "output_refs", "errors"]),
            WorkflowNode(node_id="merge", stage_id="recovery.merge", depends_on=["memory_verify"], writes=["artifact_ids", "relation_ids", "output_refs"]),
            WorkflowNode(node_id="critic", depends_on=["merge"], route_key="critic_action", loop_limit=2, writes=["pending_work_units", "errors"]),
            WorkflowNode(node_id="memory_consolidate", stage_id="memory.consolidate", depends_on=["critic"], required=False, writes=["output_refs", "errors"]),
            WorkflowNode(node_id="persist_result", depends_on=["memory_consolidate"], writes=["output_refs"]),
