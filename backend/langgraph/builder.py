...
def build_schema_recovery_graph(tool_registry: ToolRegistry, *, event_sink=None, model_gateway=None):
    builder = StateGraph(AgentState)

    builder.add_node("survey_node", lambda state: survey_node(state, tool_registry, event_sink, model_gateway))
    builder.add_node("column_node", lambda state: column_node(state, tool_registry, event_sink, model_gateway))
    builder.add_node("name_node", lambda state: name_node(state, tool_registry, event_sink, model_gateway))
    builder.add_node("code_node", lambda state: code_node(state, tool_registry, event_sink, model_gateway))
    builder.add_node("orm_node", lambda state: orm_node(state, tool_registry, event_sink, model_gateway))
    builder.add_node("skip_orm_node", skipped_orm_node)
    builder.add_node("merge_node", lambda state: merge_node(state, tool_registry, event_sink, model_gateway))
